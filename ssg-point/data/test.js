...
gungulist.map((item) => {
  ...
  // 코드와 일치 하는 시 찾기
  const si = regionList.filter((item) => item.code === code)[0].name.substring(0,2)
  if (result[si]) {
    result[si].push({code: item.code, name : item.name})
  } else {
    result[si] = [{code: item.code, name : item.name}]
  }
});
